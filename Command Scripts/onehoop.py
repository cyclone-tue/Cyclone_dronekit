...
class flight():
    ...
    def __init__(self):
        ...
        parser.add_argument(
            '--connect',
            help="vehicle connection target string. Is not necessary if --simulate is specified. It will then start a sitl.")
        parser.add_argument(
            '--simulate',
            help="Include if you want to connect to a simulation in stead of running on an actual drone. The script will take off automatically."
        )
        ...
        formatter = logging.Formatter(
            "%(asctime)s [%(threadName)-12.12s] [%(name)s-%(levelname)-5.5s] %(message)s")  # Format to use for logging.
    ...
    def end(self):
        ...
        #if self.simulation and self.sitl is not None:
    ...
    def initVision(self):
        ...
        self.vision = VisionThread(calibrationFile, self.drone, cameraID=camera, logging=self.logger,
                                   name="VisionThread")  # Initialize the vision thread
        self.vision.start()  # Start the vision thread.

    def initControl(self):
        self.high_level = HighLevelThread(name="HighLevelThread", drone=self.drone,
                                          logging=self.logger)  # TODO move to initHighlevel
