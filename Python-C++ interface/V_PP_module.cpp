...
Mat combinedEllipse3(vector<vector<Point> >& input, Mat& t) {
    ...
    //contours only of a predetermined size are considered. The upper threshold could be lowered considerably, as now our aim is to detect the individual LEDs.
...
void estimation (double omega_passed, double ell_h, double ell_w) {
    ...
    if(abs(ell_h - ell_w) < epsilon){
        omega_est = 0;
    } else {
        omega_est = omega_passed * PI / 180;    //rotation of the ellipse in radians
    }
    ...
    bb = asin(sqrt(pow(sin(omega_est),2.0)*(1-pow(k_est,2.0))));
    double temp = pow(sin(bb),2.0);
    //double temp = -sqrt(pow(sin(omega_est),2.0)*(1-pow(k_est,2.0)) - 1);
...
void display_values(){
    ...
    cout <<"dist = " << dist_meas <<"    ang_ver = " << angle_ver * 180 / PI << "    ang_hor = " << angle_hor * 180 / PI << endl;
...
void record_values(){
    ...
    angle_avg += finalEllipse[1].angle;
...
MatrixXd executeVision() {
    ...
    while (count < 1000) {
        ...
        //        cout << "fps: " << fps << endl;
        count++;
        //cout << "center of hoop in pixels is in x: " << abs(960 - finalEllipse[1].center.x * 2)<< " and in y: " << abs(540 - finalEllipse[1].center.y * 2) << endl;
        ...
        //cout << "angle is: " << angle_avg / numOfEstimates << endl;
